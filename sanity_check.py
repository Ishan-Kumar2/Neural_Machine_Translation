...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    assert(np.allclose(dec_init_state_target[0].numpy(), dec_init_state_pred[0].numpy(),rtol=1e-1)), "dec_init_state[0] is incorrect: it should be:\n {} but is:\n{}".format(dec_init_state_target[0], dec_init_state_pred[0])
    print("dec_init_state[0] Sanity Checks Passed!")
    assert(np.allclose(dec_init_state_target[1].numpy(), dec_init_state_pred[1].numpy(),rtol=1)), "dec_init_state[1] is incorrect: it should be:\n {} but is:\n{}".format(dec_init_state_target[1], dec_init_state_pred[1])
